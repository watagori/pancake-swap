...
class Transaction():
    ...
    def get_caaj(self):
        ...
        elif EvmReceipt(self.receipt_data).get_result()['type'] == "transfer":
            caaj_data = {
                "time": Header(self.header_data).get_time(),
                "platfrom": "bnb_pancakeswap",
                "transaction_id": Header(self.header_data).get_transaction_hash(),
        ...
        #         "debit_to": EvmReceipt(self.receipt_data).get_result()['from_address'],
        #         "credit_title": "SPOT",
        #         "credit_amount": {token_type[EvmReceipt(self.receipt_data).get_result()[
        #             'from_token_address'][0]]: EvmReceipt(self.receipt_data).get_result()['from_token_amount'][0],
        #             token_type[EvmReceipt(self.receipt_data).get_result()[
        #                 'from_token_address'][1]]: EvmReceipt(self.receipt_data).get_result()['from_token_amount'][1]},
        #         "credit_from": EvmReceipt(self.receipt_data).get_result()['from_address'],
        #         "credit_to": EvmReceipt(self.receipt_data).get_result()['to_address'],
